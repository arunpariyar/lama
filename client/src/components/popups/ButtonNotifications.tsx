...
function ButtonNotifications({ user, setUser }: ButtonNotificationsProps) {
  ...
  useEffect(() => {
    if (user) {
      setEmailWhenDue(user && user.notif_due || false);
      setEmailWhenFreq(user && user.notif_opt || false);
      setEmailFreq(user && user.notif_freq || "");
    }
  }, [user]);
